...
describe('createWebSocketHandler – a stopped tool call', () => {
  ...
  it('ignores a tool_interrupted frame with no tool_call_id', () => {
    // Without the falsy-id guard every ordinary user and assistant message
    // (which carry no tool_call_id) would be rewritten to a stopped tool row.
    const deps = makeDeps()
    const handler = createWebSocketHandler(deps)

    handler({ type: 'tool_interrupted', tool_name: 'calc_add' })

    const rows = deps.mapMessages.mock.calls.at(-1)[0]([
      { role: 'user', content: 'hello' },
      { tool_call_id: 'call-1', status: 'calling' },
    ])
    expect(rows[0]).toEqual({ role: 'user', content: 'hello' })
    expect(rows[1].status).toBe('calling')
  })
