...
- **Pod logs view**: The run detail "Pod logs" tab now parses the agent runner's newline-delimited JSON and offers three view modes — Table (time / kind / message columns, the default), Text (a readable block of `[time] [kind] message` lines), and Raw (the original JSON) — with `kind`-based coloring so warnings, errors, and final answers stand out. Non-JSON lines are preserved verbatim.
- **In-cluster MCP egress**: `build_network_policy` now detects an in-cluster MCP host (`*.svc`, `*.svc.cluster.local`, or a bare service name) among the run's resolved MCP servers and adds a same-namespace egress allowance. Previously the RFC1918 except-clause blocked the cluster Service range, so an in-cluster MCP server was unreachable from the pod and the agent's connection attempt hung until the HTTP timeout. Public MCP/LLM egress is unchanged, and runs with no in-cluster MCP get no extra allowance.
- **Demo MCP server**: added `atlas/agent_runner_v3/demo_mcp/` — a stdlib-only Streamable-HTTP MCP server (tools: `get_project_secret_code`, `multiply`) plus `k8s.yaml`/`deploy.sh` to run it in-cluster on the agent image for end-to-end testing. `mcp.json` ships an `mcp_tools_demo` entry pointing at it. The long-lived demo pod also pins the agent image against kubelet image-GC on disk-pressured dev nodes (which otherwise evicts the local-only image and yields `ImagePullBackOff`).
- **Network egress self-check**: a new "Run network egress self-check" toggle on the launch form (request field `egress_check`, env `ATLAS_EGRESS_CHECK`) makes the agent probe a mix of allowed (public HTTPS) and blocked (cloud-metadata `169.254.169.254`, RFC1918) destinations from inside the pod and log each as an `egress` line, demonstrating the per-run NetworkPolicy. Public HTTP(S) is permitted by design; private/link-local ranges and non-80/443 ports are blocked.
- **Egress allowlisting (Phase 0, off by default)**: an admin can restrict what a launched agent may reach to an explicit allowlist, denying everything else. New settings `FEATURE_AGENT_PORTAL_V3_EGRESS_ALLOWLIST_ENABLED`, `AGENT_PORTAL_V3_EGRESS_MODE` (`required_allowlist`|`user_choice`|`open`), `AGENT_PORTAL_V3_EGRESS_ALLOWLIST`, `AGENT_PORTAL_V3_EGRESS_USER_ALLOWLIST_MAX`. When enabled, `build_network_policy` emits a deny-by-default policy (no `0.0.0.0/0`): DNS + the allowlist domains resolved to `ipBlock` `/32`s on 80/443 + same-namespace pods for in-cluster MCP. The run's LLM host and selected MCP hosts are always added; the launch form shows a locked notice and `/capabilities` exposes the policy. Phase 0 resolves domains to IPs at launch (CDN-IP drift and wildcards are deferred to the Phase 1 egress gateway / OpenShift `EgressFirewall`). See `docs/agent-portal-v3/egress-allowlisting.md`.
