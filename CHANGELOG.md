...
### PR #818 - 2026-08-18
- **The test suite no longer shares state between tests or with the developer's checkout**: four leaks, each reproduced before being fixed. (1) `test_capability_tokens_and_injection` replaced `sys.modules["atlas.modules.llm.litellm_caller"]` with a fake at import time, so every module imported afterwards bound `_FakeLLM` -- running that file before `test_llm_retry` broke collection and before four other files failed 86 tests; the suite survived only because `test_llm_env_expansion` sorts earlier and undid it, one of four copies of the same workaround. The stub was unnecessary and is gone, along with all four workarounds. (2) The agent-portal e2e fixture cleared `pm_mod._singleton_manager`, a name that does not exist, so the ProcessManager and its process table survived across tests and `test_launch_list_get_cancel_round_trip` failed under reverse or shuffled order. (3) Importing `main` binds the process-wide engine to the real `data/chat_history.db` (`AppFactory()` is built at import time), so runs accumulated real rows in the developer's chat-history and agent-portal databases, wrote feedback JSON into `runtime/feedback/`, and deleted `logs/security_high_risk.jsonl`; `conftest.py` now redirects every such store into a per-session temp directory and `prompt_risk` resolves its log through a new `high_risk_log_path()` that honors `APP_LOG_DIR` like the other logs already do. (4) `GLOBUS_AUTH_BASE_URL`, `ATLAS_ENV_FILE`, a CI-only `sys.path` entry and three `patcher.start()` calls on shared functions all leaked past the tests that set them. Adds an autouse fixture restoring 17 singleton module globals, guard tests for both classes of regression, and `docs/developer/test-isolation.md`.
