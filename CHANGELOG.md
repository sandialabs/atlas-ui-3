...
### Follow-up to #867 - 2026-08-28
- **In-app links in LLM chat responses now stay in the current tab, and the `target` allowlist is hardened** (review feedback on #867): #867 made external links open in a new tab, but it only exempted `#`-prefixed hrefs from `target="_blank"`, so a link like `[settings](/settings)` still opened a second copy of ATLAS. A new `isExternalHref` helper in `frontend/src/utils/markdownRenderer.js` now keeps in-app navigation (server-relative `/settings`, relative `./`/`../`, query strings, and same-origin absolute URLs) in the current tab; only protocol-relative (`//host`) and cross-origin URLs open new. Separately, because `DOMPurify.ADD_ATTR` is element-agnostic, raw model HTML such as `<a target="_top">`, `<a target="_blank">` without a `rel`, or `target` on a non-anchor could have survived the widened allowlist; a DOMPurify `afterSanitizeAttributes` hook (registered once at module load) now keeps `target` only on `<a>`, normalizes any anchor target to `_blank`, forces `rel="noopener noreferrer"`, and strips `target` from everything else -- closing a `target="_top"`/clickjacking path and a tabnabbing path. The vitest suite `frontend/src/test/llm-link-target.test.js` grows from 5 to 11 tests with per-anchor DOM assertions covering the external, in-app, fragment, protocol-relative, raw-HTML-anchor, and non-anchor cases.
