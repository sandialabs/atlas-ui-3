...
### PR #867 - 2026-08-28
- **Links in LLM chat responses now open in a new tab instead of navigating away from ATLAS** (closes #859): the marked link renderer already emitted `target="_blank" rel="noopener noreferrer"` on external links, but the rendered HTML is sanitized with `DOMPurify.sanitize(html, DOMPURIFY_CONFIG)` before being injected into assistant messages, and DOMPurify 3.x strips the `target` attribute by default (it is no longer in the built-in allowlist). Only `rel="noopener noreferrer"` survived, so every link opened in the same tab. `target` is now explicitly added to `DOMPURIFY_CONFIG.ADD_ATTR` in `frontend/src/utils/markdownRenderer.js`, so the sanitizer preserves the `target="_blank"` the renderer emits. External links and bare/autolinked URLs open in a new tab. A new `isExternalHref` helper also keeps in-app navigation (server-relative `/settings`, relative `./`/`../`, query strings, and same-origin absolute URLs) in the current tab -- previously a `[settings](/settings)` link would have opened a second copy of the app -- while fragment links (`#section`, used by citation-badge / references scrolling) continue to stay same-tab.
- **The widened `target` allowlist is constrained by a DOMPurify `afterSanitizeAttributes` hook** (review feedback): `ADD_ATTR` is element-agnostic, so without a guard raw model HTML such as `<a target="_top">` or `<a target="_blank">` without a `rel`, or `target` on a non-anchor element, would survive sanitization. The hook (registered once at module load in `frontend/src/utils/markdownRenderer.js`) keeps `target` only on `<a>`, normalizes any anchor target to `_blank`, and forces `rel="noopener noreferrer"` -- closing a `target="_top"`/clickjacking path and a tabnabbing path. New vitest suite `frontend/src/test/llm-link-target.test.js` covers external/in-app/fragment/protocol-relative links and the raw-HTML anchor + non-anchor cases with per-anchor assertions.
