...
### PR #828 - 2026-08-21
- **`atlas-chat` CLI can launch in headless agent mode** (closes #827): the CLI hardcoded `agent_mode=False` when calling `client.chat()`, so agent mode was unreachable from the command line even though the backend fully supported it. A new `--agent-mode` flag (`action="store_true"`) is registered in `build_parser()` and passed through as `agent_mode=args.agent_mode` in `run()`. The orchestrator already routes to `AgentModeRunner` (the agentic loop with `tool_choice="auto"`) when `agent_mode=True` and tools are selected, falls back to plain mode with a warning when no tools are selected, and strips agent mode when the model lacks tool support — so no backend changes were needed. Usage: `atlas-chat "Write a script" --agent-mode --tools server_tool1`.
