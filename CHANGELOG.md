...
### PR #744 - 2026-08-05
- **Lifecycle hook / plugin system**: Added a server-side plugin bus (`atlas/core/hooks/`) so governance behavior -- auditing tool calls, redacting prompts, forcing approval for a class of tools, rewriting a RAG query, dropping high-risk chunks -- can be installed by an operator instead of edited into `orchestrator.py`, `tool_executor.py`, and `unified_rag_service.py`. Eight hook points fire at existing chokepoints (SessionStart, UserPromptSubmit, PreLlmCall, PreToolUse, PostToolUse, PermissionRequest, RagCall, RagResponse), with a `CONTINUE`/`MODIFY`/`DENY`/`REQUIRE_APPROVAL` return contract. `MODIFY` is a structured patch restricted to each event's declared mutable fields, so trusted context (session, user, compliance level) is never patchable, and it is the only way to change an event -- a handler that edits one in place and returns `CONTINUE` has its edit discarded rather than applied unvalidated. Composition is most-restrictive-wins and independent of plugin ordering: an escalation to approval cannot be lowered by a later handler, and a chain that both patches and escalates still delivers the patch. Plugins may tighten a boundary but never widen one: argument injection and schema filtering are re-applied after a PreToolUse patch so `_atlas_user` cannot be forged, RAG source lists are narrow-only, admin-mandated approval cannot be auto-approved away, a SessionStart context patch cannot seed runtime-owned session keys, a PreLlmCall patch that repoints the model is re-authorized against the turn's user with the same per-model group check the orchestrator applies, and a PostToolUse denial withholds the tool's artifacts along with its text. PreLlmCall fires on every provider round-trip (each agentic-loop iteration, each tool-synthesis call, streaming and not) and sees the fully assembled request -- system prompt, history, injected RAG context, tool results -- so a plugin can inspect or rewrite what actually leaves the process, pin the model or temperature, or refuse the call. A denial's `reason` is operator-facing (logs and spans only); a plugin passes `user_message` for anything the end user should read. Handlers run under a timeout with per-hook-point fail-closed/fail-open defaults -- sync handlers included, via a worker thread, so a blocking plugin cannot stall the event loop, and plugin loading is fail-fast and idempotent (`AppFactory` is built more than once per process; the registry is not). Off by default (`FEATURE_HOOKS_ENABLED`); every call site short-circuits when no plugin is registered. Documented in `docs/developer/hook-plugin-system.md` (closes #713).
