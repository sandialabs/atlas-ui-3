...
### PR #781 - 2026-08-12
- **Reasoning content streams from reasoning-capable LLMs**: models that emit chain-of-thought before their answer (o-series, Qwen3, GPT-OSS via vLLM) had all of it discarded. The streaming layer now yields a reasoning marker per delta plus a final block with the full text, which the mode runners forward as `reasoning_token` / `reasoning_content` events and persist in assistant message metadata; the frontend renders them in a collapsible per-message section that auto-expands while streaming and survives a reload. Two ordering fixes come with it: a turn that reasons and then goes straight to tool calls now closes its token stream, so post-tool synthesis starts a fresh message instead of appending to the reasoning bubble; and the local-save debounce keys on `messages` rather than `messages.length`, since content and reasoning stream in without changing the array length and the save was firing mid-stream against a truncated message. Includes a self-delimited monkey-patch for [BerriAI/litellm#20246](https://github.com/BerriAI/litellm/issues/20246) (vLLM's `delta.reasoning` is dropped during LiteLLM chunk conversion); see `docs/developer/reasoning-content-streaming.md` for removal instructions. Replaces PR #655.
