...
### PR #682 - 2026-06-29
- **Selectable Atlas RAG tools**: Added Atlas RAG pseudo MCP tools (`atlas_rag_discover_data_sources`, `atlas_rag_query`) and surfaced them as an `atlas_rag` pseudo-server in the tools panel (via `/api/config`) so users explicitly choose whether the model can discover/query RAG sources. Selecting RAG sources alone does not enable these tools — the model can only call them when the user selects them in the tools panel. They work in both ordinary tools mode and agent mode (honoring the user's selected sources in both, and otherwise querying all sources the user can access). `atlas_rag_query` only queries sources in the user's discovered/authorized set, so a model- or client-supplied `data_sources` list cannot reach a configured source the user is not authorized for. `compliance_level` on the discover tool is an advisory display filter, not an authorization boundary (group membership is the boundary).
