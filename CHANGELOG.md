...
### PR #673 - 2026-06-23
- **Compact tool-approval messages**: Tool-approval prompts rendered inside the full `System` bubble and, with auto-approve on, opened their argument JSON expanded by default — a single auto-approved call (e.g. `pptx_generator_markdown_to_pptx` with a long `markdown_content`) took more vertical space than the tool-call output below it, and the collapse choice reset to expanded on every render and reload. The prompt now renders as a compact row matching the tool-call style: `tool_approval_request` routes through the same avatar-less/header-less/bubble-less path as `tool_call`, the arguments panel collapses to a single line, and the choice persists to `localStorage` so it sticks across messages and reloads (F5). With no saved preference, auto-approved calls start collapsed (informational) while approval-required calls start expanded (reviewable); an explicit toggle then wins globally. Removed the dead `ToolApprovalDialog.jsx` (a modal variant no longer rendered anywhere) and its test. See `docs/developer/design-notes/compact-tool-system-messages-2026-06-23.md`.
