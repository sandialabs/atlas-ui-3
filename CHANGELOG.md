...
- **Transfer MCP no longer sandboxes to a base directory by default**: The transfer server is a local single-developer helper, so its `read_file_from_disk` / `write_file_to_disk` tools now read and write anywhere the process can reach. Relative paths still resolve below `MCP_TRANSFER_BASE_DIR` (or the working directory), but absolute paths outside it are allowed instead of returning `Access denied: path outside base directory`. The previous sandboxing is preserved as an opt-in: set `MCP_TRANSFER_RESTRICT_TO_BASE_DIR=true` to confine all access to the base dir. Documented the transfer env vars (`MCP_TRANSFER_BASE_DIR`, `MCP_TRANSFER_MAX_BYTES`, `MCP_TRANSFER_RESTRICT_TO_BASE_DIR`) in `.env.example`.
