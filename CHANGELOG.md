...
### PR #673 - 2026-06-23
- **Compact tool-approval messages**: Tool-approval prompts now render as a compact transcript row matching tool calls (no `System` bubble), with the arguments panel collapsing to a single line and the choice persisted across messages and reloads; auto-approved calls default collapsed, approval-required default expanded. Removed the dead `ToolApprovalDialog.jsx` and its test. See `docs/developer/design-notes/compact-tool-system-messages-2026-06-23.md`.
- **Compact-messages toggle**: Added a *Compact Tool Messages* switch under Settings → General (on by default). Turning it off restores the classic per-message layout — avatar, author header, and expanded bubbles — for tool calls, approval prompts, logs, agent meta, and system notices. The choice persists in `localStorage` alongside the other user settings.
