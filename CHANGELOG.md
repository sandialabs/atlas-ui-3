...
### PR #743 - 2026-08-05
- **Agent Skills discovery and injection**: Atlas discovers skill directories written to the [Agent Skills specification](https://agentskills.io/specification) and appends their `name`/`description` index to the system prompt, behind `FEATURE_SKILLS_ENABLED` (default off). Skills merge across layered roots -- packaged `atlas/config/skills`, then `~/.atlas|.claude|.agents/skills`, then the same three under the project root -- with later roots overriding same-named skills; reading the `.claude` and `.agents` locations means skills authored for other Agent Skills tools work without being duplicated, and `SKILLS_PATHS` replaces the default list. The index is appended *after* template formatting and *additively* to whichever system prompt won, so it composes with a user-selected custom prompt instead of being dropped by it, and a brace in a skill description cannot reach `str.format()`. The skill list is rescanned when a new session starts, so adding a skill on disk takes effect without a server restart. A malformed `SKILL.md` is logged and skipped rather than failing the scan.
- **Read-only skills MCP server**: New opt-in `skills` server (`atlas/config/mcp-example-configs/mcp-skills.json`) exposes `list_skills`, `read_skill`, and `read_skill_resource` so the model can load a skill's full instructions once it decides the skill applies. It only reads files -- skill-bundled `scripts/` are never executed -- and resource reads are confined to the skill's own directory and capped at 256 KB.
