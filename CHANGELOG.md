...
- Disconnecting a server no longer reports a failure for a disconnect that succeeded: the token is already gone by then, so the revoked catalogue is unpublished unconditionally and a cached-client cleanup error is logged rather than raised.
...
- Disconnect fails closed: a failure to withdraw the per-user catalogue is reported rather than hidden behind a 200 that leaves the revoked tool names and schemas published and routable. Dropping the cached client stays a quiet best-effort, since it discloses nothing.
...
- A disconnect withdraws the catalogue before deleting the token, so a failed withdrawal is retryable: the token is still stored, the error says the disconnect did not happen, and the retry takes the same path instead of short-circuiting on 404 with the catalogue published and the provider token live.
