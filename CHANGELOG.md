...
### PR #769 - 2026-08-10
- **In-range npm audit fixes for frontend (lockfile-only)**: `npm audit fix` (no `--force`) in `frontend/` resolved 6 of 11 flagged packages, all non-major and inside existing `^` ranges so `package.json` is unchanged: `@babel/core` (low, arbitrary file read via sourceMappingURL), `brace-expansion` (high, DoS via unbounded/zero-step expansion), `flatted` (high, unbounded-recursion DoS in `parse()`), `form-data` (high, CRLF injection via unescaped multipart field names), `picomatch` (high, method injection in POSIX character classes), and `ws` (high, uninitialized memory disclosure). The remaining 5 (`vitest` critical, `vite` high, `@vitest/mocker`/`esbuild`/`vite-node` moderate) all share a single semver-major fix (`vitest` 2 -> 4.1.10) and are deferred to a separate upgrade PR; `npm audit fix` correctly refuses to cross a major without `--force`, and `vitest` is dev-only with no production dev-server exposure per `AGENTS.md`. Verified: `npm run lint` (0 errors), `npm test` (521/521), `npm run build` (success), `npm audit` (11 -> 5 packages).
