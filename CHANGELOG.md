...
### PR #835 - 2026-08-22
- **Reopening a conversation from history re-enables its workspace** (closes #829): the active workspace id is persisted with each conversation and restored on load, so you no longer re-pick the prompt, sources, and tools. Best effort — a deleted workspace is silently skipped, a workspace-less conversation leaves the active one untouched, and a restore that lands before the workspace list loads is deferred until it does. Applies to both server- and browser-saved conversations.
