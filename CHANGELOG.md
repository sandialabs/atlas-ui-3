...
### PR #669 - 2026-06-22
- **Fixed stateful MCP session errors in agent mode**: Stateful MCP servers (HTTP/SSE servers that keep per-session state, e.g. an `Mcp-Session-Id`) raised session errors in agent mode while working fine in regular chat. The cause was that the agentic loop never carried a `conversation_id`: `AgentContext` had no such field and `AgenticLoop` built its `session_context` from only `session_id`/`user_email`/`files`. With `conversation_id=None`, `MCPToolManager.call_tool` took the single-use-session branch and opened/closed a fresh MCP session for every tool call, so stateful servers lost their session between the loop's sequential calls. Regular tools mode was unaffected because it builds context via `build_session_context(session)`, which spreads `session.context` (including the `conversation_id` that `ChatService` always populates, defaulting to `str(session_id)`). The agent path now mirrors that: `AgentContext` carries `conversation_id`, agent mode populates it from the session, and the loop forwards it so `MCPSessionManager` reuses one persistent session across all tool calls. Added a regression test asserting the loop forwards `conversation_id` to the tool manager.
