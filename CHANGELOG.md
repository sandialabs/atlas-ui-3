...
### PR #773 - 2026-08-11
- **Removed the email domain whitelist feature**: `FEATURE_DOMAIN_WHITELIST_ENABLED`, `atlas/core/domain_whitelist.py`, `atlas/core/domain_whitelist_middleware.py`, `atlas/config/domain-whitelist.json`, and `docs/admin/domain-whitelist.md` are gone. The feature was unused, and its loader searched obsolete paths that did not include the documented project override, the package default, or `APP_CONFIG_DIR` — so when enabled it typically found no file, and `is_domain_allowed()` returned `True` on that path, admitting every domain. A restriction that silently allows everything is worse than no restriction, and removal resolves it outright.
- **`uv.lock` is back in sync with `pyproject.toml`**: `uv export --locked` had been failing, and the stale lock resolved versions *below* the floors declared in `pyproject.toml` — `reportlab` sat at 4.4.10 under a `>=5.0.0` constraint. 38 packages moved.
- **Two security checks now actually block a merge**: the dependency scans all used `continue-on-error` or `|| true`, so a green "Security Checks" only meant a scan had run. Lockfile consistency and high/critical advisories in *shipped* frontend dependencies (`npm audit --omit=dev`) now fail the build. The remaining scanners stay advisory on purpose — they cover the whole dev environment, where most findings are build tooling that never reaches a user, and gating on those teaches people to ignore the job.
- **Cleared all advisories from shipped frontend dependencies**: an in-range `npm audit fix` took `npm audit --omit=dev` to zero. The 5 remaining advisories in the full tree are all `vitest`/`vite`/`esbuild` build tooling that needs a major bump and does not ship to a browser. New [dependency-advisories.md](docs/developer/dependency-advisories.md) documents what blocks, why, and how to record an accepted advisory with an expiry instead of weakening the gate.

### PR #772 - 2026-08-11
- **`FEATURE_TOOLS_ENABLED` is now a real boundary**: the flag only filtered what `/api/config` advertised, so a crafted client could still submit `selected_tools`, `selected_prompts`, or `agent_mode` over the WebSocket and have them honoured — hiding tools in the UI was not a mitigation for anything. The orchestrator now discards all three when the flag is off (covering programmatic callers as well as the socket), and MCP client initialization, tool discovery, and the auto-reconnect loop are skipped entirely at startup rather than connecting to servers nothing is allowed to use.
- **MCP sampling enforces the per-model group ACL**: the sampling routing context carried no user identity, so the handler ran whichever model the MCP server asked for — or simply the first configured one — without the group check that normal chat applies, letting a server reach a model the calling user is denied. Identity is now threaded into `_SamplingRoutingContext`, the server's preference is honoured only if it passes `check_model_access`, an unauthorized request falls back to an authorized model, and a request with no authorized model is denied rather than downgraded. `max_tokens` is clamped to the new `MCP_SAMPLING_MAX_TOKENS` (default 4096), since the value came from the MCP server and was a direct cost lever.
- **One chat turn per WebSocket**: each `chat` frame created a background task and overwrote the single task reference without cancelling the previous one, so a client sending several frames left earlier turns running with no handle on them — still making metered model calls, still calling tools, and racing each other writing session history, while stop/reset could only cancel the most recent. Additional frames are now rejected with a `turn_in_progress` error while a turn is active. HTTP rate limiting never covered this path, being HTTP middleware.
- **`agent_max_steps` is clamped server-side**: the value arrived off the WebSocket frame with no ceiling, and each step is a metered model call plus its tool calls. It is now bounded by the configured `agent_max_steps`, with non-integer and out-of-range values falling back to the default.
- **Development-only MCP servers refuse to start in production**: `file_viewer`, `filesystem`, `code-executor`, `code_executor_v2`, and `transfer` grant arbitrary host file access or outbound fetches to any authenticated user. Enabling one while `ENVIRONMENT=production` or `DEBUG_MODE=false` now aborts startup, mirroring the Agent Portal gate. Detection is by server name so copying an example config is caught, with `"dev_only": true/false` available to mark a custom server or to opt out one you have reviewed.
