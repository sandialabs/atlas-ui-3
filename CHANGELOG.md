...
### PR #871 - 2026-08-30
- **The Active Tools strip above the chat input stays a single row on mobile** (#870): the strip's pill row used `flex-wrap`, so on a narrow viewport the "Active Tools:" label, the tool pills, the "+N more" toggle and the auto-approve button stacked into up to three rows between the transcript and the composer. The pill row is now `flex-nowrap` and scrolls horizontally when it overflows (`overflow-x-auto scrollbar-hide`, the same pattern as the follow-up suggestion buttons), pills no longer shrink, and the "+N more" and auto-approve toggles moved out of the scroll region to pinned slots at the end of the row so they can never be scrolled out of reach; the scroll region is keyboard-focusable for overflow access (review). Verified in a browser at 375px: exactly one row (26px) with 3, 9 compacted and 9 expanded tools; desktop layout unchanged.
