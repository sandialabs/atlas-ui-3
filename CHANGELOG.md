...
### PR #835 - 2026-08-22
- **Reopening a conversation from history re-enables its workspace** (closes #829): the active workspace id now travels with each chat turn and is persisted in the conversation's metadata, so loading an old conversation switches back to the workspace it was tied to instead of leaving the user to re-pick it. The switch is best effort — a workspace that has since been deleted is silently skipped, and a conversation that never had a workspace leaves the currently active one untouched. If the workspace list has not finished loading when the conversation opens, the switch is deferred until it does, so a slow fetch no longer drops the restore. Conversations saved before this change carry no workspace id and behave as before.
