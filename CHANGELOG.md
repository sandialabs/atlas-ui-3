...
- The Tools panel lists a server that requires authorization even when it has discovered no tools yet, so its connect control is reachable, and the "Tools appear after you connect" hint is shown for every server that offers a connect control (`api_key`/`bearer`/`jwt` as well as `oauth`) and only while the user is not connected.
- A promoted per-user catalogue records every user it came from, so one of them disconnecting republishes from a remaining authorized user rather than leaving the others' tools listed but unroutable.
- A per-user catalogue expires: once past its TTL it is neither served to its owner nor left published, so a user whose token stops working stops being shown the tools it used to justify, and a re-discovery that comes back empty withdraws what the same user published before.
- Tool descriptions and input schemas from a server that gates `tools/list` are no longer handed to other users' model context: `get_tools_schema` and `build_mcp_data` take the requesting user and omit tools that user did not discover, and that read access expires with the per-user entry rather than outliving it.
- Connecting or disconnecting a server by pasting or removing a token now re-fetches the tool catalogue, so the tools it unlocks appear (and the ones it withdraws disappear) without a page reload; the OAuth flow already did this by returning through a full page load.
- `get_tools_schema` fails closed on an unknown user, matching `build_mcp_data`: an omitted or `None` user withholds every user-scoped catalogue, and the internal callers that resolve an already-authorized tool say so with an explicit `UNSCOPED` sentinel.
- A per-user discovery whose credential is withdrawn while it is still running discards its result instead of republishing the catalogue the disconnect had just withdrawn.
- `_mcp_data` emits the requesting user's own object for a tool on a co-owned server, so two owners of a same-named tool no longer cross-disclose descriptions and input schemas.
- Disconnect withdraws the per-user catalogue before deleting the token and fails closed: the revoked tool names and schemas cannot stay published and routable behind a 200, and because the token is still stored when the withdrawal runs, a failure is retryable rather than leaving the catalogue published with no API path to clear it. Dropping the cached client afterwards stays a quiet best-effort, since it discloses nothing and its failure must not report a disconnect that happened as an error.
- A per-user catalogue past its TTL is withdrawn even while the server is in its failure cool-down, instead of staying published for the length of the window.
- The internal "no user to scope against" marker is an identity-compared sentinel object rather than a string, so a header-derived `user_email` cannot take its value and switch off scoping.
- A tool withheld from the schema because its per-user catalogue is not the requester's (expired, or another user's) is logged rather than silently dropped, so a turn that runs without a selected tool is diagnosable.
- Withdrawing the last promotion for a server clears its task-support metadata, instead of letting the departed owner's `taskSupport` verdicts decide for whatever is discovered next.
...
### PR #903 - 2026-09-10
- Review follow-ups to parallel conversation runs (#884), all on the run-vs-connection boundary:
  - **Elicitation responses are now bound to their owner.** An `ElicitationRequest` carries the authenticated `user_email` it was created for and rejects a mismatched response fail-closed, mirroring the existing `ToolApprovalManager` check. Previously any authenticated user holding an elicitation id could inject data into another user's tool execution -- and because a background run may wait indefinitely, the id stayed answerable for the life of the process rather than for a five-minute window.
  - **Files produced by a background run can be downloaded.** `download_file` searches the connection session first and then the sessions of the requesting user's runs, because a tracked run registers its files on its own session; every such download previously failed with "File not found in session". The search only ever covers runs owned by the requester, and the per-file user check is unchanged.
  - **Files attached before a background turn reach it.** `attach_file` writes to the connection session, which the run's fresh session could not see, so a just-attached file was invisible to the very turn sent to act on it. The file map is now copied across at run admission.
  - **Closing a second tab no longer tears down a live run's MCP sessions.** The release decision is taken from the run registry rather than the closing connection's own run set, so a run started in another tab keeps the MCP clients it is still using.
  - **A cancelled approval wait no longer leaks its pending request.** The wait is wrapped in `try/finally`; cancellation (user Stop, or the wall-clock sweeper) unwound through neither the success nor the timeout cleanup, retaining the request and its arguments permanently -- and with `TOOL_APPROVAL_TIMEOUT_SECONDS=0` cancellation is the only exit, so nothing cleaned up at all.
  - **A background run's errors render in their own conversation.** Every frame `handle_chat` emits, including the error branches and the cancellation flush, now goes through the run-tagging callback instead of a raw `websocket.send_json`, so an untagged rate-limit or timeout error is no longer spliced into whichever conversation is on screen.
  - **A stale stop frame cannot cancel an unrelated turn.** Stop/`agent_control` decide the untracked fallback from whether the frame addressed a run, not from whether a cancel took effect -- the two were indistinguishable, so a stop naming an already-finished run fell through and cancelled the connection's current untracked turn.
  - Run cleanup is shielded so a second cancellation cannot skip session release, and Atlas warns when `TOOL_APPROVAL_TIMEOUT_SECONDS=0` and `MAX_RUN_WALL_CLOCK_SECONDS=0` are combined, which leaves a run paused on approval with no expiry.
...
- **Seven review follow-ups on the combined panel**: a tab requested before `/api/config` resolves (the marketplace return trip asks for Tools) is now held and applied the moment its feature flag makes it visible, instead of being silently replaced by General for the rest of the session. The Prompts tab is kept mounted behind `hidden` like the tools tab, so a half-written prompt survives a tab switch, and a prompt-edit intent is cleared once it has opened its editor rather than re-applying every time the tab is re-entered. The tools tab's **Save Changes** / **Cancel** commit or revert and stay, since dismissing from a per-tab button also exited Prompts, General, User Info and Admin -- dismissal is reserved for the header X, the backdrop and Escape; the standalone modal still closes on save. **Full Admin Page** goes through the same unsaved-selection guard as the X. The still-mounted pending selection is re-seeded when the saved selections change underneath it and is intersected with the live tool list on save, so the Admin tab's **Reload MCP** pruning keys for a vanished server is no longer undone. The panel gained `role="dialog"` / `aria-modal`, a `tablist` with `aria-selected` and arrow-key navigation, focus move on open, a focus trap and Escape-to-close -- it is now the only route to tools, theme and admin controls. Admin notification timers are cleared on unmount and the admin tab stays mounted after its first visit, so save results are actually seen.
- **Second review round on the combined panel**: "Full Admin Page" now defers `navigate('/admin')` until the unsaved-selection guard is actually answered (and drops the navigation if the user backs out), instead of navigating straight through the prompt it had just raised. Escape and the focus trap are scoped to the innermost open dialog -- the unsaved-changes prompt, token input and admin config editor each declare `role="dialog"`, so Escape over a half-entered token no longer tears down the whole panel, and the trap uses `getClientRects()` rather than `offsetParent` (null inside a `position: fixed` ancestor, which is exactly how those modals render). An in-progress prompt draft reports dirty like tool selections do and gets its own discard/keep-editing prompt. The tools tab's re-seed compares selection contents instead of `Set` identity, so a context that republishes fresh Sets cannot loop. Initial focus is keyed on open alone, so toggling a tool no longer yanks focus to the Close button. `aria-controls` is omitted for tabs whose panel is not mounted, the embedded save confirms in place now that it does not dismiss, and `loadSystemStatus` checks `response.ok`.
- **Third review round on the combined panel**: saving the tools tab no longer drops a saved selection for a server that is simply absent from the current tool list -- the live-key intersection now filters *additions* only, and removals are computed against the unpruned pending set, so an MCP reload that has republished some servers but not others cannot cost the user their selection on the next unrelated save. Escape and Tab handling moved back to the document, scoped by event target, because clicking non-focusable text drops focus to `document.body` -- after which Escape did nothing and Tab walked the page behind the modal; focus is now pulled back into the panel instead. The admin notification stack is portalled to the body, since a `hidden` tabpanel is `display: none` and was swallowing every save result from any other tab. Discarding a prompt draft no longer clears the child's dirty flag, so a discard whose close is then aborted by the tools guard asks again on the next close rather than silently destroying the draft. The General tab's **Cancel** defers its revert behind the close, so an aborted close no longer throws away in-progress General settings. Adds direct `useAdminConfigActions` coverage for the `banners` / `help-config` / default request shapes, the error path, and timer cleanup.
- **Approval nits on the combined panel**: Escape and Tab no longer switch off while a nested overlay is open. Each overlay owns its own Escape through a new `useEscapeKey` hook (`UnsavedChangesDialog`, `TokenInputModal`, `AdminModal` and the disconnect confirm), and the panel's Tab trap is scoped to the innermost open dialog rather than standing down -- previously focus could walk out of both overlays and Escape was a no-op on the unsaved-changes prompt. The embedded footer's **Cancel** is now **Discard Changes** and routes through the same confirmation as every other exit, instead of silently throwing away staged selections. `ToolsPanel` is wrapped in `React.memo` and its server merge/sort moved into a `useMemo` keyed on the tool and prompt lists, so the always-mounted tools tab no longer re-merges the whole server tree on every General-tab slider step.
- **UX review follow-ups (cmlanca)**: the admin shield is gone from the top bar -- admin controls are the Admin tab and the full dashboard is one click on from there -- and Help and Portal are icons rather than icon-plus-word. The model picker moved out of the header and under the message box, joining new chat-bar controls for tools (a searchable menu of every tool as icon + name + one-line description + on/off toggle, instead of a flat list of names) and a pill strip showing which datasets are enabled, each removable, collapsing to `+N more` past three. Data sources are only ever read by the search tool, so the picker was rendered inline in the Tools & Integrations tab as well as in its own drawer -- both share a new `DataSourcesSelector` -- and dataset names wrap instead of being clipped mid-word. The left-hand header button labels are driven by the header's own measured width rather than a viewport media query, which is what left "New Chat" stuck at the wrong size when the sidebar or canvas panel changed the space available. The prompt picker's "Custom Prompts" heading, which read as a selectable row, is now a muted uppercase "Add Custom Prompts" label.
- **Dataset pill strip matches the Active Tools strip above it**: after merging #871, the new Data Sources pills were still on the old wrapping layout, so on a narrow viewport they wrapped into extra rows directly under a strip that had just been fixed not to. They now use the same single-row treatment -- `flex-nowrap` with horizontal scroll, an icon-only label below `sm`, and the `+N more` toggle outside the scroll region so it stays reachable.
