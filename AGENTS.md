...
All security checks must fail closed: when a security check encounters an error, missing configuration, or unexpected state, it must deny access rather than grant it. Never return a permissive default on exception. If a config file is missing or corrupt, deny access and log an error. If an authorization check throws, deny the request. This applies to tool authorization, domain whitelisting, compliance level enforcement, capability token generation, and any other access control path.
