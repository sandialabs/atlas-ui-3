...
**WelcomeScreen Layout**: On landscape (wide-and-short) screens the WelcomeScreen switches from a vertical stack to a horizontal row (`landscape:flex-row`) so the logo appears on the left and help text on the right, preventing overlap.
