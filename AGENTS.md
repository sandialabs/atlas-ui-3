...
### FIPS-Compatible Session Signing

Application and test code must import `SessionMiddleware` from `atlas.core.session_middleware`. Do not use Starlette's session middleware directly: its SHA-1 signer default is unavailable under FIPS policy. The Atlas wrapper signs with SHA-256.

When resolving merges, preserve the Atlas wrapper import rather than accepting a direct `starlette.middleware.sessions` import. Run `atlas/tests/test_fips_session_signing.py` and the affected browser-auth route tests after session-middleware changes to prevent regressions.
