...
class TestMCPSessionManager:
    ...
    @pytest.mark.asyncio
    async def test_acquire_same_conversation_different_users_get_different_sessions(
        ...
        s1 = await session_manager.acquire(
            "conv-1", "server-a", client1, user_email="alice@test.com"
        )
        s2 = await session_manager.acquire(
            "conv-1", "server-a", client2, user_email="bob@test.com"
        )

        assert s1 is not s2

        await session_manager.release_all("conv-1", user_email="alice@test.com")
        client1.__aexit__.assert_called_once()
        client2.__aexit__.assert_not_called()

        await session_manager.release_all("conv-1")
        client2.__aexit__.assert_called_once()
