...
class UnifiedRAGService:
    ...
    async def _run_rag_call_hooks(
        ...
        if chain.modified:
            if "query" in chain.patched_fields and event.query != query:
                messages = _replace_query_text(messages, event.query)
            if "data_sources" in chain.patched_fields:
    ...
    async def _run_rag_response_hooks(
        ...
        if chain.modified:
            if "content" in chain.patched_fields:
                response.content = event.content
            if "metadata" in chain.patched_fields:
                response.metadata = event.metadata
            elif "content" in chain.patched_fields:
                # Content was redacted but the plugin did not also patch
                # metadata. The original metadata carries the retrieved chunk
                # text that ``_format_rag_references`` renders verbatim into the
                # References section, so it must not survive the redaction.
