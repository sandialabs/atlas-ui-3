...
class HookRegistry:
    ...
    async def dispatch(self, event: HookEvent) -> HookChainResult:
        ...
        for registration in registrations:
            ...
            if result.decision == HookDecision.MODIFY:
                applied = self._apply_modify(registration, event, result)
                if applied is not None:
                    ...
                    if registration.fail_open:
                        ...
                        continue
                    chain.decision = HookDecision.DENY
                    chain.reason = f"hook {registration.name} returned an invalid patch: {applied}"
    ...
    def _apply_modify(
        ...
        """Validate and apply a MODIFY patch. Returns an error string on reject."""
