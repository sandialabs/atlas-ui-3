...
class LiteLLMStreamingMixin:
        ...
        with start_span("llm.call", span_attrs) as span:
            ...
            try:
                ...
                async for chunk in response:
                    ...
                    # When the first content token arrives, emit the final ReasoningBlock
                ...
                # Reasoning-only stream (no content followed): still emit the block
    ...
    async def stream_with_tools(
        ...
        an LLMResponse with accumulated tool_calls at the end.
        """
        if not tools_schema:
        ...
        with start_span("llm.call", span_attrs) as span:
            ...
            try:
                ...
                # Stream ended on reasoning with no content (e.g. the model went
                # straight from reasoning to tool calls): still emit the block.
                if accumulated_reasoning and not reasoning_emitted:
                    yield ReasoningBlock(content=accumulated_reasoning)
