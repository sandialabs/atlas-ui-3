...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Public Demo MCP Server",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python main.py                    # Run as stdio (default)
  python main.py --http             # Run as HTTP server on port 8002
  python main.py --http --port 9000 # Run as HTTP server on port 9000
  python main.py --sse              # Run as SSE server on port 8002
        """
    )
    parser.add_argument(
        "--http",
    ...
    parser.add_argument(
        "--sse",
        action="store_true",
        help="Run as SSE server instead of stdio"
    )
    parser.add_argument(
        "--port",
        type=int,
        default=8002,
        help="Port for HTTP/SSE server (default: 8002)"
    )
    parser.add_argument(
        "--host",
        type=str,
        default="0.0.0.0",
        help="Host for HTTP/SSE server (default: 0.0.0.0)"
    )
    ...
    if args.http:
        import uvicorn
        print(f"Starting Public Demo MCP server (HTTP) on {args.host}:{args.port}")
        print(f"MCP endpoint: http://{args.host}:{args.port}/mcp")
        uvicorn.run(mcp.http_app(), host=args.host, port=args.port)
    elif args.sse:
        import uvicorn
        print(f"Starting Public Demo MCP server (SSE) on {args.host}:{args.port}")
