...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="JWT Demo MCP Server",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python main.py                    # Run as stdio (default)
  python main.py --http             # Run as HTTP server on port 8001
  python main.py --http --port 9000 # Run as HTTP server on port 9000
  python main.py --sse              # Run as SSE server on port 8001
        """
    )
    parser.add_argument(
        "--http",
    ...
    if args.http:
        import uvicorn
        print(f"Starting JWT Demo MCP server (HTTP) on {args.host}:{args.port}")
        print(f"MCP endpoint: http://{args.host}:{args.port}/mcp")
        uvicorn.run(mcp.http_app(), host=args.host, port=args.port)
    elif args.sse:
        import uvicorn
        print(f"Starting JWT Demo MCP server (SSE) on {args.host}:{args.port}")
