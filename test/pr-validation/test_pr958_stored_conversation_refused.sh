#!/bin/bash
# Test script for PR #958: refuse a stored conversation id the caller does
# not own before a run is admitted.
#
# Covers the issue's repro end to end:
#   1. User A runs an agent turn that completes and saves (a stored record).
#   2. User B names A's conversation id on an agent turn: refused with an
#      `authorization` error, no run_started/run_status frames.
#   3. No run record exists for the refused turn in B's runs snapshot.
#   4. A's stored conversation is untouched; B cannot read it over REST.
#   5. A plain turn naming the foreign id is refused as before (no run path).
#   6. Control: B's own new-chat agent run is still admitted and completes.
#   7. Backend unit suite.
#
# A real backend is started on a free port with a scripted OpenAI-compatible
# mock model (fixtures/pr958/mock_llm.py) that returns atlas_sleep tool calls,
# so agent turns are admitted as tracked runs and saved exactly as they are in
# the app. Nothing on the path under test is stubbed.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
FIXTURES="$SCRIPT_DIR/fixtures/pr958"
...
print_header "1-6. Stored-conversation ownership before run admission, over the real WebSocket and REST API"
