...
deployment. Paste this into a file and apply with
`oc apply -f openshift-telemetry.yaml`. Creates the namespace,
ConfigMaps, a PVC for Tempo, three Deployments (`otel-collector`,
`tempo`, `grafana`), their Services, and a `Route` exposing Grafana.

Assumes OpenShift 4.x with the default `restricted-v2` SCC. If the Tempo
or Grafana pods crash with permission errors on startup, the cluster
admin can grant the `anyuid` SCC to the namespace's default service
account (`oc adm policy add-scc-to-user anyuid -z default -n atlas-telemetry`).
The manifest below uses `fsGroup` so the PVC mount is writable by the
arbitrary UID OpenShift assigns.
...
data:
  config.yaml: |
    ...
    exporters:
      otlp/tempo:
        endpoint: tempo.atlas-telemetry.svc:4317
        tls:
          insecure: true
    service:
      pipelines:
  ...
  datasources.yaml: |
    ...
    datasources:
      - name: Tempo
        type: tempo
        access: proxy
        url: http://tempo.atlas-telemetry.svc:3200
        isDefault: true
---
apiVersion: v1
...
spec:
  ...
  template:
    metadata:
      labels: { app: tempo }
    spec:
      # fsGroup is left unset so the restricted-v2 SCC admission controller
      # assigns one from the namespace range — hardcoding a value would be
      # rejected unless the service account has the anyuid SCC.
      containers:
        - name: tempo
          image: grafana/tempo:latest
          args: ["-config.file=/etc/tempo/tempo.yaml"]
          ports:
            - { name: query, containerPort: 3200 }
            - { name: otlp-grpc, containerPort: 4317 }
          volumeMounts:
      ...
      volumes:
        ...
        - name: data
          persistentVolumeClaim: { claimName: tempo-storage }
---
apiVersion: v1
kind: Service
metadata:
...
spec:
  selector: { app: tempo }
  ports:
    - { name: query, port: 3200, targetPort: 3200 }
    - { name: otlp-grpc, port: 4317, targetPort: 4317 }
---
apiVersion: apps/v1
kind: Deployment
