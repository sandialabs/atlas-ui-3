...
- Reloads `mcp.json` from `config/` (with package defaults in `atlas/config/` as fallback).
...
Use this after editing `config/mcp.json` to apply changes **without restarting** the application.
