...
Accepted values are `none`, `minimal`, `low`, `medium`, `high` and `xhigh` — the set LiteLLM will carry on a chat/completions request. A value outside that set (a typo like `meduim`, or an unquoted `None`, which YAML reads as the *string* `"None"`) raises a configuration error when ATLAS loads `llmconfig.yml`; it is not swallowed by the loader's empty-model fallback. The raised `ConfigurationError` names the offending field and the configuration search paths, rather than the problem surfacing as a provider `400` during someone's chat. Configuration values are not echoed back except for the specific scalar that failed, so a bad entry cannot spill `api_key` or `extra_headers` into logs. The failure is remembered for the life of the process: once `llmconfig.yml` is rejected, every later read raises the same error, so **correct the file and restart** — an in-process retry will report the original problem. Surrounding whitespace is trimmed and a blank value is treated as unset.
