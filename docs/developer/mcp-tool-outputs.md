...
*   **`_atlas_user` Injection (Security Feature)**: If your tool's function signature includes an `_atlas_user: str` parameter, the backend will **always overwrite** any value for it with the authenticated user's identity. This is a critical security feature to ensure a tool always runs with the correct user context. Legacy `username` injection is temporarily supported with a deprecation warning; new tools should use `_atlas_user` so ordinary `username` arguments remain under LLM control.
