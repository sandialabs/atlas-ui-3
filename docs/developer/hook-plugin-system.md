...
turn via a `ContextVar` (`atlas.core.hooks.hook_turn`) and the RAG events read
it from there. For RAG events the `user_email` field comes from the
`UnifiedRAGService` call's `username` parameter (the caller's own identity),
while `session_id`, `conversation_id`, and `compliance_level` are sourced from
the per-turn `ContextVar`. A RAG query issued outside a chat turn carries `None`
for the session-scoped fields.
...
context, selected tool/source lists), so an in-place edit of
`event.arguments["options"]["path"]` cannot reach the call site behind a
`CONTINUE`.

For events with more than one mutable field, `HookChainResult.patched_fields`
records exactly which fields an accepted `MODIFY` touched. Call sites consume
only those fields, so a handler that mutates one field in place (behind a
`CONTINUE`) cannot smuggle its edit in alongside an accepted patch on a different
field. A `RAG_RESPONSE` plugin that redacts `content` should also patch
`metadata` (or let the runtime clear it): `_format_rag_references` renders
chunk text from `metadata` into the References section, so unredacted metadata
would disclose the very text the plugin removed. When `content` is patched but
`metadata` is not, the runtime drops `metadata` automatically.
