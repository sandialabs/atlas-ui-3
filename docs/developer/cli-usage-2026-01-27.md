...
python atlas_chat_cli.py --list-data-sources --json \
    ...
    --config-overrides /path/to/config/overrides \
    --rag-sources-config /path/to/config/overrides/rag-sources.json \
    --llm-config /path/to/config/overrides/llmconfig.yml
...
- `config/defaults/` for LLM, MCP, and RAG config (with optional overrides via `APP_CONFIG_OVERRIDES` or `--config-overrides`)
- MCP tool discovery runs automatically on first use
- Tool servers are defined in `config/defaults/mcp.json` (or your overrides directory)

For scripted testing and CI, you can override config locations directly via CLI flags (e.g., `--config-overrides`, `--llm-config`, `--mcp-config`, `--rag-sources-config`). These are applied before loading `.env` and before importing Atlas modules, so they take precedence over values set in `.env`.
