...
*   **`api_key`**: (string) The API key for authenticating with the model's provider. It's best practice to use environment variable substitution here (e.g., `${MY_API_KEY}`).
...
*   **`extra_headers`**: (dictionary) A set of custom HTTP headers to include in the request, which is useful for some proxy services or custom providers.
