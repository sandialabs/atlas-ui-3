...
*   **Modify `RAGMCPService.discover_servers`:**
    *   Change the method signature in `atlas/domain/rag_mcp_service.py` from `discover_servers(self, username: str)` to `discover_servers(self, username: str, user_compliance_level: Optional[str] = None)`.
...
*   **Modify `RAGMCPService.discover_data_sources`:**
    *   Change the method signature in `atlas/domain/rag_mcp_service.py` from `discover_data_sources(self, username: str)` to `discover_data_sources(self, username: str, user_compliance_level: Optional[str] = None)`.
    *   This new parameter will also carry the compliance level selected by the user.

#### 2. Implement Server Filtering

*   **Location:** Inside both `RAGMCPService.discover_servers` and `RAGMCPService.discover_data_sources` in `atlas/domain/rag_mcp_service.py`.
...
*   **Access Check:** Obtain an instance of `ComplianceLevelManager` using `get_compliance_manager()` from `atlas/core/compliance.py`.
...
*   **Location:** Within `RAGMCPService.discover_servers` in `atlas/domain/rag_mcp_service.py`, specifically after retrieving the `resources` (individual data sources) from an accessible server.
...
*   **Location:** The `get_config` function in `atlas/routes/config_routes.py`, which handles the `/api/config` endpoint.
...
*   **Location:** Inside the `get_config` function in `atlas/routes/config_routes.py`.
